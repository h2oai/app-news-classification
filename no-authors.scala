...
val data = sc.textFile("/Users/avniwadhwa/Desktop/ExperimentalNewOtherTechCrunch.csv").filter(x => !isHeader(x)).map(d => d.split(',')).filter(row => row.length == 5 && row(4) != "")
...
val XXXwords = data.map(d => (d(2), token(d(4)).toSeq)).filter(s => s._2.length > 0)
val words = XXXwords.map(v => v._2)
val XXXlabels = XXXwords.map(v => v._1)


// YYYY
val words = data.map(d => (d(2), token(d(4)).toSeq, token(d(2).toSeq))).filter(s => s._2.length > 0 || s._3.length > 0)
val YYYlabels = words.map(v => v._1)
val YYYwords = words.map(v => v._2)
...
case class TECHCRUNCH(target: String, a: mllib.linalg.Vector)

import org.apache.spark.h2o._
val h2oContext = new H2OContext(sc).start()
import h2oContext._

val resultRDD: DataFrame = XXXlabels.zip(title_vectors).map(v => TECHCRUNCH(v._1, v._2)).toDF
